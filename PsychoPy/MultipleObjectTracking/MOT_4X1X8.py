#Here, a Graphical User Interface (GUI) is designed for a unique-target multiple
#object tracking (MOT). Namely, every MOT trial starts
#with the 500-ms display of an empty gray square of
#20 × 20° of visual angle on a black background. Then, four
#white discs (filled, diameter 1.3° of visual angle) appear
#at random positions (but all different) inside the square
#and one disc (the target) immediately starts flashing for
#1s (the disc turns red four times with 200-ms intervals,
#remaining red for 400 ms the fourth time). The target
#become white again, and all discs start to follow random
#strait paths inside the frame for 8 s at 8° of visual
#angle (deg/s). The discs are reflected with the same
#angle as the incidence one whenever they touched an
#edge. They further remain still at their latest position
#until receiving the child’s response. The mouse pointer
#appeared on the screen, and the participant could select
#the target using the wireless mouse located on their dominant
#side. Disc selection immediately starts the next
#trial. 

#This code is a modification of the OpenSource code shared by 
#Meyerhoff HS, Papenmeier F. Individual differences in visual attention:
#a short, reliable, open-source, and multilingual test of multiple object
#tracking in PsychoPy. Behav Res Methods. 2020;52:2556–66.

#You can uncomment adecuate line code to: 
#(1) add practice trials
#(2) add more targets
#(3) add or remove distractor discs
#(4) add feedback to the user about correct and incorrect answer
#(5) inform the user when a new set of trials will start


#You can also adjust the "blocks" of motion paths to add sets of trials as needed. 
...
#Only the 5 first trials aof the first block re run here to reduce computation time, but adjust nTrialsPerBlock to your needs
#The last 2 trials led to incorrect answers and all others to correct ones.

expMonitorName="testMonitor"
monitorFPS=60 #Any deviation from 60 Hz requires new motion paths

#params
nTrialsPerBlock=5 #Adjust to what is needed
#nPracticeTrials=5
nObjects=4 #do not change (more than 8 requires different trial files)
nTargets=1 #do not change (other than 4 not tested)
...
#Load different font for Asian languages (otherwise they appeared cropped on Mac systems in our tests)
fontInstructions = ''
...
def doTrial(m):
    ...
    while np.sum(arrayMarkiert[0,:]) <1: 
        maus.clickReset()
        # wait for mouse down
        while True:
...
def getSubjectData():
    ...
    info.addField('Number of Blocks:', initial = '15', choices = ['1','4']) 
    info.addField('Age:')
    ...
    while True:
        ...
        subIdExistsAlready = os.path.isfile('results/MOT'+str(vpData[0])+'_'+str(vpData[1])+'.txt')
...
def saveShortLogfile(subId, data):
    ...
    np.savetxt('results/MOT'+str(vpData[0])+'_'+str(vpData[1])+'_short.txt', dataShort, delimiter=";", fmt="%s")
    
    return observedCapacity

#Funcion para definir el texto para el participante 
...
def showInstructions(mywin, number_duration_minutes):
    ...
    instrMainText = _('¡Hola!')   
    instrMainText = instrMainText + '\n\n'
    instrMainText = instrMainText + _('Este juego dura alrededor de {number_duration_minutes} minutos. Podrás jugar mientras tendrás los audífonos puestos.').format(number_duration_minutes = number_duration_minutes)
    instrMainText = instrMainText + '\n\n'
    instrMainText = instrMainText + _('Al inicio de cada prueba, 4 discos blancos aparecerán en la pantalla. Uno de ellos se pondrá a parpadear varias veces con un color rojo.')
    instrMainText = instrMainText + '\n\n'
    instrMainText = instrMainText + _('¡Fíjate cuál disco es y no lo pierdas de vista! Este disco volverá a su color blanco y todos los discos empezarán a moverse.')
    instrMainText = instrMainText + '\n\n'
    instrMainText = instrMainText + _('Tu objetivo es no perder de vista el disco que parpadeó en rojo. Cuando los discos dejen de moverse, haz clic en ese disco. ')
    instrMainText = instrMainText + '\n\n'
    instrMainText = instrMainText + _('¡Mucha suerte!')
    instrMainText = instrMainText + '\n'
    ...
    pngParadigm = visual.ImageStim(mywin, "illustrationParadigm_4.png", units="deg")
    ...
    #paradigmFeedbackText.setText(_('{numcorrect} out of 1 correct').format(numcorrect = 1)) 
    ...
    instrL1.setText(_('Si tienes dudas, ¡pregúntame!'))
    ...
    instrL2.setText(_('-- Presiona cualquier tecla para empezar --'))
...
#practice trials
#showInfoScreen(_('Practice'), infoScreenFreezeInterval) #Comenté esa línea para que ya no haya distinción entre los trials de prática y de exámen
#for nPra in range(nPracticeTrials):
    ...
    #tarCorrect = np.sum(arrayMarkiert[0,0:1]) 
    #showInfoScreen(_('{numcorrect} out of 1 correct').format(numcorrect = int(tarCorrect)), infoScreenFreezeInterval)
    
# Block- and Trial-Loop
#showInfoScreen(_('Start of the test'), infoScreenFreezeInterval) #Comenté esa línea para que ya no haya distinción entre los trials de prática y de exámen
...
np.savetxt('results/MOT'+str(vpData[0])+'_'+str(vpData[1])+'.txt', data, delimiter=";", fmt="%s", encoding='utf-8')
observedCapacity = saveShortLogfile(vpData[0], data)
showFinalScreen(_('Fin del juego'), 2, _('Avisa al investigador que ya terminaste'), "",observedCapacity)
maus.setVisible(1)
mywin.close()
core.quit()