...
% Modified from original code of Dong and colleagues (line 364: adjust path)
...
handles.cfg.RetainChannsFlag = 0;   % Retain remaining channels?
...
i = [];  ALLEEG = evalin('base','ALLEEG'); k = size(ALLEEG,2);
assignin('base','k',k); 
for i = 1:1:k
    EEG1 = []; 
    EEG1 = ALLEEG(i);
    ...
    % Set channels 
    chan = [1:1:length(EEG1.chanlocs)];
    assignin('base','chan',chan); 
    names = []; j = []; names = [];
    for j = 1:1:length(EEG1.chanlocs)
        b = convertCharsToStrings(EEG1.chanlocs(j).labels);
        names = [names;b];
    end
    assignin('base','names',names);
    
    handles.cfg.SelectChanns.chanlist = chan;
    handles.cfg.SelectChanns.cellchannames = names;
    
    % REST     
    try  evalin('base','EEG1'); 
         if ~isempty(EEG1.data)
            ...
            if ~isempty(handles.cfg.InputLeadfieldDir)
                ...
                if isfield(EEG1.chanlocs,'X') && isfield(EEG1.chanlocs,'Y') && isfield(EEG1.chanlocs,'Z')
                    if ~isempty(EEG1.chanlocs(1).X) && ~isempty(EEG1.chanlocs(1).Y) &&~isempty(EEG1.chanlocs(1).Z)
                        channs = []; channs = handles.cfg.SelectChanns.chanlist; % selected channs
                        xyz_elec = []; xyz_elec = zeros(length(channs),3);
                        for nc = 1:length(channs)
                            xyz_elec(nc,1) = EEG1.chanlocs(channs(nc)).X;
                            xyz_elec(nc,2) = EEG1.chanlocs(channs(nc)).Y;
                            xyz_elec(nc,3) = EEG1.chanlocs(channs(nc)).Z;
                        end
                    else
            ...
            try disp(['Current data set: ',EEG1.setname]);catch;end; 
            OrigData = [];
            if ~isempty(handles.cfg.SelectChanns.chanlist)
                channs = handles.cfg.SelectChanns.chanlist;
                if length(size(EEG1.data)) == 3
                    OrigData = EEG1.data(channs,:);
            ...
            if size(OrigData,1) == size(G,2)        
                ...
                %assignin('base', 'REST_data_i', REST_EEG);

                if length(size(EEG1.data)) == 3
                   REST_EEG = reshape(REST_EEG,size(REST_EEG,1),size(EEG1.data,2),size(EEG1.data,3));
                   disp('********Reshape to channels X timepoints X epochs!!!!');
                end
                handles.cfg.REST_EEG = REST_EEG;       % EEG data refered to REST
                % Update handles structure
                guidata(hObject, handles);
                %handles.cfg.REST_EEG
                disp('Completed...');
            else
    ...
    if ~isempty(handles.cfg.REST_EEG)
        ...
        if flag1 == 0
            ...
            EEG1.data = handles.cfg.REST_EEG;
            %assignin('base', 'EEG1_data_i', EEG1.data);
            EEG1.ref = 'REST';
            try
            [~, Origdatfile1, ext] = fileparts(EEG1.datfile);
            EEG1.datfile = [Origdatfile1,'_REST',ext];
            catch
                disp('''EEG.datfile'' may be empty!');
            end
            try
            [~, Origdatfile2, ext] = fileparts(EEG1.filename);
            EEG1.filename = [Origdatfile2,'_REST',ext];
            catch
                disp('''EEG.filename'' may be empty!');
            end
            try
            [~, Origdatfile3, ext] = fileparts(EEG1.setname);
            EEG1.setname = [Origdatfile3,'_REST',ext];
            catch
                disp('''EEG.setname'' may be empty!');
            end
            EEG1.chanlocs = EEG1.chanlocs(handles.cfg.SelectChanns.chanlist);
            EEG1.nbchan = length(handles.cfg.SelectChanns.chanlist);
            EEG1.comments = 'Re-referencing to REST';
            EEG1.ref = 'REST';
            % ----------------
        else
            % Retain un-selected channels (e.g. EMG, EOG, bad channels etc)
            if length(size(EEG1.data)) == 3
                EEG1.data(handles.cfg.SelectChanns.chanlist,:,:) = handles.cfg.REST_EEG;
            else
                EEG1.data(handles.cfg.SelectChanns.chanlist,:) = handles.cfg.REST_EEG;
            end
            EEG1.ref = 'REST';
        ...
        pop_saveset(EEG, 'filename',[EEG.filename],...
            'filepath', 'C:\Users\mathi\OneDrive\Documents\Job\GitHub\MATLAB\Codes\'); 
...
% MOVIDO a Run_pushbutton3_Callback
